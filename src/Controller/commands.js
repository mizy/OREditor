...
const getAllCommands = (editor,actions)=>{
    ...
    return [
        ...
        {// 换行，把当前元素后面的全部插入新的p组件，然后重绘
            name:"Wrap",
            keys:[{ key:'Enter', altKey: false, ctrlKey: false, shiftKey: false }],
            execute : function(){
                ...
                if(nowIndex===0){
                    ...
                    const oldStr = component.spliceChar(nowIndex,component.data.length-nowIndex);
                    data.children.push({
                        ...component.toJSON(),
                        data:oldStr
                    });
        ...
        {
            name:"Left",
            keys:[{ key:'ArrowLeft', altKey: false, ctrlKey: false, shiftKey: false }],
            execute : function(){
                ...
                if(index>0){
                    activeComponent.index--;
                        ...
                        renderer.activeComponent.index = renderer.activeComponent.data.length-1;
        ...
        {
            name:"Right",
            keys:[{ key:'ArrowRight', altKey: false, ctrlKey: false, shiftKey: false }],
            execute : function(){
                const {activeComponent} = renderer;
        ...
        {
            name:"Up",
            keys:[{ key:'ArrowUp', altKey: false, ctrlKey: false, shiftKey: false }],
            execute : function(){
                const {activeComponent} = renderer;
        ...
        {
            name:"Down",
            keys:[{ key:'ArrowDown', altKey: false, ctrlKey: false, shiftKey: false }],
            execute : function(){
                const {activeComponent} = renderer;
                const {nowLine,parent} = activeComponent;
        ...
        },
        {
            name:"SetStyle",
            keys:[],
            execute : function(style){//删掉对应的片段然后再新增带样式的片段
        ...
        {
            name:"SelectAll",
            keys:[{ key:'a', altKey: false, ctrlKey: true, shiftKey: false },
            { key:'a', altKey: false, metaKey:true,ctrlKey: false, shiftKey: false }],
            execute : function(){
               const {section,cursor} = page;
               const start = renderer.children[0].children[0];
               const end = renderer.children.slice(-1)[0].children.slice(-1)[0];
